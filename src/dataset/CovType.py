...
class CovType:
    ...
    def get_converted_converted_dataset_resampled(self, dataloader=False, test_size=None, random_state=None, batch_size=None, fraction=1.0):
        ...
        y_test_upsampled_tensor = torch.tensor(y_test.values, dtype=torch.long)
    ...
    def get_concept_drift_dataset(self, dataloader=False, test_size=None, random_state=None, batch_size=None, train_fraction=0.25, test_fraction=1.0):
        ...
        y_test_drifted_tensor = torch.tensor(y_test.values, dtype=torch.long)

        X_train_drifted_tensor = torch.tensor(X_train_drifted.values, dtype=torch.float32)
        y_train_drifted_tensor = torch.tensor(y_train.values, dtype=torch.long)
...
#     covtype = CovType()
#     covtype.get_converted_dataset()
#     covtype.get_converted_converted_dataset_resampled()
...
#     print(f"Number of categorical features: {len(covtype.cat_cols)}")